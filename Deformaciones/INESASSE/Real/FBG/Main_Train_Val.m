...
Folder_Path = 'Data\';
File_Name = 'Mean-1to10_Min-Load-30.mat';
...
Data_Tr = LSTM(2).Ty_Si_St_Tr;
Labels_Tr = LSTM(2).Ty_Si_La_Tr;
Data_Val = LSTM(2).Ty_Si_St_Val;
Labels_Val = LSTM(2).Ty_Si_La_Val;
Data = LSTM(2).Ty_Si_St_Te;
Labels = LSTM(2).Ty_Si_La_Te;

for i = 1:length(Data_Tr)
    Data_Tr{i} = Data_Tr{i}';
end
for i = 1:length(Data_Val)
    Data_Val{i} = Data_Val{i}';
end
for i = 1:length(Data)
    Data{i} = Data{i}';
end
